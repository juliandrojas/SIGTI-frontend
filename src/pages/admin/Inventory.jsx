...
export default function Inventory() {
  ...
  const editItem = (item) => {
    setForm({ ...emptyForm, name: item.name || "", category: "component", brand: item.brand || item.reference || "", quantity: Number(item.quantity || 0), available_quantity: Number(item.available_quantity || 0) });
  ...
  const submit = async (event) => {
    ...
    setSaving(true);
    try {
      const payload = { name: form.name.trim(), category: "component", brand: form.brand.trim(), reference: form.brand.trim(), quantity, available_quantity: availableQuantity };
  ...
  return <div className="container py-4">
    ...
    {showForm && <div className="card shadow-sm border-0 mb-4"><div className="card-body"><div className="d-flex justify-content-between align-items-center mb-3"><h2 className="h5 mb-0">{editingId ? "Editar artículo" : "Registrar artículo"}</h2><button type="button" className="btn-close" aria-label="Cerrar" onClick={resetForm} /></div><form onSubmit={submit} className="row g-3">
      ...
      <div className="col-md-6"><label className="form-label" htmlFor="component-brand">Marca o referencia</label><input id="component-brand" className="form-control" value={form.brand || ""} onChange={(e) => setForm({ ...form, brand: e.target.value })} /></div>
    ...
    <div className="card shadow-sm border-0"><div className="card-body"><div className="row g-2 mb-4"><div className="col-12 position-relative"><label className="visually-hidden" htmlFor="inventory-search">Buscar por nombre o marca/referencia</label><input id="inventory-search" className="form-control" placeholder="Buscar por nombre o marca/referencia" value={search} autoComplete="off" onChange={(event) => setSearch(event.target.value)} />{suggestions.length > 0 && <div className="list-group position-absolute w-100 shadow-sm" style={{ zIndex: 10 }} role="listbox" aria-label="Sugerencias de búsqueda">{suggestions.map((suggestion) => <button key={suggestion} type="button" className="list-group-item list-group-item-action" onMouseDown={(event) => event.preventDefault()} onClick={() => setSearch(suggestion)}><i className="bi bi-search me-2 text-muted" aria-hidden="true" />{suggestion}</button>)}</div>}</div></div>{loading ? <p className="text-muted">Cargando...</p> : filteredItems.length === 0 ? <p className="text-muted mb-0">No hay artículos con esos filtros.</p> : <div className="table-responsive"><table className="table table-hover align-middle mb-0"><thead><tr><th>Elemento</th><th>Marca / referencia</th><th>Total</th><th>Disponibles</th><th className="text-end">Acciones</th></tr></thead><tbody>{filteredItems.map((item) => <tr key={item.id}><td className="fw-semibold">{item.name}</td><td>{item.brand || item.reference || "-"}</td><td>{item.quantity}</td><td>{item.available_quantity}</td><td className="text-end"><button className="btn btn-sm btn-outline-primary me-1" type="button" onClick={() => editItem(item)}>Editar</button><button className="btn btn-sm btn-outline-danger" type="button" onClick={() => remove(item)}>Eliminar</button></td></tr>)}</tbody></table></div>}</div></div>
