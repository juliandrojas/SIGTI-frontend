...
const emptyForm = {
  ...
  quantity: 1, available_quantity: 1, condition: "good", location: "bodega", notes: "",
};
...
export default function Inventory() {
  ...
  const editItem = (item) => {
    setForm({ ...emptyForm, name: item.name || "", category: "component", brand: item.brand || "", reference: item.reference || "", model: item.model || "", serial_number: item.serial_number || "", quantity: Number(item.quantity || 0), available_quantity: Number(item.available_quantity || 0), condition: item.condition || "good", location: item.location || "bodega", notes: item.notes || "" });
  ...
  return <div className="container py-4">
    ...
    {showForm && <div className="card shadow-sm border-0 mb-4"><div className="card-body"><div className="d-flex justify-content-between align-items-center mb-3"><h2 className="h5 mb-0">{editingId ? "Editar artículo" : "Registrar artículo"}</h2><button type="button" className="btn-close" aria-label="Cerrar" onClick={resetForm} /></div><form onSubmit={submit} className="row g-3">
      ...
      <div className="col-md-6"><label className="form-label" htmlFor="component-location">Ubicación</label><input id="component-location" className="form-control" value={form.location || ""} onChange={(e) => setForm({ ...form, location: e.target.value })} /></div>
