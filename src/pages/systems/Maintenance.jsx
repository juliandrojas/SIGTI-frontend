...
export default function Maintenance() {
  ...
  const load = async () => {
    try {
      const [itemsResponse, recordsResponse] = await Promise.all([api.get("/inventory/items"), api.get("/inventory/maintenance")]);
  ...
  useEffect(() => {
    ...
    const loadInitial = async () => {
      try {
        const [itemsResponse, recordsResponse] = await Promise.all([api.get("/inventory/items"), api.get("/inventory/maintenance")]);
        if (active) { setItems(itemsResponse.data); setRecords(recordsResponse.data); }
